...
The backend matches the host (ADR-0044): a Cursor SDK agent in Cursor (browser
login via `init`, ADR-0038), or the `claude` CLI in Claude Code (the machine's
existing sign-in — detection, never a pasted key). Live activity and
finished/failed results are stored under
