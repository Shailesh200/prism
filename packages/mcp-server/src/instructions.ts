...
- pause / resume / cancel / add context to a job → job_control (canonical id or title). Pause freezes the teammate; it does not kill it. Resume continues the same process. If resume returns needsConfirm because the checkout gained uncommitted files the job has not seen, relay that and re-call with confirmDirty=true only when the user agrees.
...
- “configure Dispatch” / standup layout / Slack channels / Linear vs Jira → configure. Settings apply to every repository on this machine, and to Cursor, Claude Code, Codex, and Claude Desktop.
