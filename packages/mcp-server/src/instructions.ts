...
- “start working on …” + a ticket/PRD → start_job. Return the tool message immediately (do not wait for the worker to finish). Each job is a teammate in its own worktree. Tell the user to say where are we for live status and the result when it finishes or fails. If a Cursor login page opened, tell the user to finish it. If “Authenticating prism…” with Skip appears, tell them to click Skip and retry — do not wait on that spinner. If start_job says Prism does not see a git repository, reload the prism MCP server while this git project is the open folder, then retry. Do not ask the user to paste a path into mcp.json. Only mention PRISM_WORKSPACE if a reload still fails. Do not start_job for “find issues” / audit / health — that is repository_health.
