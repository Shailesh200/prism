import type { RepositoryMap } from "@prism/shared";
...
describe("deriveRegions", () => {
  ...
  it("gives a perfect score to a region with no edges", () => {
    const regions = deriveRegions(
      graph([{ id: "x", kind: "folder", label: "docs" }], []),
    );
    expect(regions[0]!.score).toBe(100);
    expect(regions[0]!.files).toBe(0);
  });
});
