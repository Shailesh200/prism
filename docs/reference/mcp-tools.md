...
| [`job_control`](#job_control) | Pause, resume, retry, reverify, cancel, delete, confirm, add context to, commit, keep, or restore files for a Dispatch job |
...
Arguments: `title`, `prd`, `jobId`, `branch`, `placement`, `confirmDirty`, `workspace`, `playbook` (`skill` writes a Prism skill globally and skips dirty-tree confirmation and repo checks for that job only), `workerModel`, `confirmOverlap`, `parentJobId`, `origin`.
