...
Hash router in `packages/dispatch-hub/src/dashboard/router.ts`. Hosted at `http://prismhq.localhost:17330/`.

| Hash | View | What it renders |
|---|---|---|
| `#/dashboard` | Dashboard | Fleet Timeline / Board / List + Focus inspector (`JobsScreen`) |
| `#/attention` | Attention | Cards waiting on the user (confirm / stalled / paused) |
| `#/findings` | Findings | Write-ups from finished jobs |
| `#/iris` | Iris | Spectrum map (`RepositoryMapView`) |
| `#/settings` | Settings | Local Dispatch prefs |

Legacy: `#/jobs` → dashboard, `#/workflows` → attention, `#/intelligence` → iris.

Focus inspector opens as a right Drawer over Dashboard when a job is selected (`?job=`).
Compose New job is a Drawer with footer `Queue job`.
