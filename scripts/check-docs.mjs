...
for (const file of files) {
  ...
  // Code spans and shell blocks are where a doc *claims* something is a
  // command. Prose says things like "toggle prism off" or "say prism init"
  // (an MCP chat phrase, not a CLI command), which are not claims about the
  // CLI and must not be read as one.
  const codeText = [
  ...
  for (const snippet of codeText) {
    for (const match of snippet.matchAll(/\bprism ([a-z][a-z-]*)/g)) {
      const name = match[1];
